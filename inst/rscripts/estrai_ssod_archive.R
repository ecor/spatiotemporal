...
## ---------------------------------------------------------------------------
## (Opzionale) scaricare uno o piu' file dall'archivio
## ---------------------------------------------------------------------------
# es.: scarica il README
# readme_url <- file_df$url[grepl("README", file_df$key)]
# download.file(readme_url, destfile = "README.txt", mode = "wb")

# es.: scarica tutti i .tar.gz in una cartella locale "ssod_archive/"
# dir.create("ssod_archive", showWarnings = FALSE)
# tar_files <- file_df[grepl("\\.tar\\.gz$", file_df$key), ]
# for (i in seq_len(nrow(tar_files))) {
#   dest <- file.path("ssod_archive", basename(tar_files$key[i]))
#   download.file(tar_files$url[i], destfile = dest, mode = "wb")
# }
