...
years=1979:2025
gsod_files="/home/ecor/local/data/climate/gsod/tanganyika_v3/station_gsod_%s.csv"
gsod <- list()
for (year in years) {
  
  print(id)
  nn <- sprintf("Y%s",as.character(year))
  filename <- gsod_files |> sprintf(nn)
  cond <- file.exists(filename)
  download_cond <- !cond
  if (cond) {
    gsod[[nn]] <- filename |> read_csv() ###read.table(filename,header=TRUE,sep=",",quote=FALSE)
  } else if (download_cond) {  
    gsod[[nn]] <- get_GSOD(station=ids,year=year) |> try(silent=TRUE)
    if (is(gsod[[nn]],"try-error")) {
      gsod[[nn]] <- NULL
    } else if (is.data.table(gsod[[nn]])) {
      write.table(gsod[[nn]],file=filename,quote=FALSE,sep=",",col.names=TRUE,row.names = FALSE)
    }
  }
}
### VERIFFY
#a <- (gsod[[nn]]==gsod_nn) | (is.na(gsod[[nn]]) & is.na(gsod_nn))
# summary(a)
...
####
db_file <- "/home/ecor/local/rpackages/jrc/spatiotemporal_material/data/db_tanganyika.rda"
