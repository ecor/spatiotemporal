...
function(input, output, session) {
  output$map <- renderLeaflet({
    ...
    iselectedp <- which(db$p$provider_id %in% input$gauge_station_provider_id)
    if (length(iselectedp)<1) iselectedp <- 1:nrow(dp$p)
    ll <- leaflet() %>% addScaleBar(position="bottomleft") %>%
      addProviderTiles(providers[[input$basemap]]) %>%
      addMarkers(data = db$p[iselectedp,], popup = ~paste("Name:", name2))
